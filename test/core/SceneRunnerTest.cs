    ...
    class SceneRunnerTest
    {
        ...
        public async Task RunScene_ColorCycle()
        {
            ...
            await AssertThrown(runner.AwaitSignal("panel_color_change", box1, Colors.Yellow).WithTimeout(700))
                .ContinueWith(result => result.Result?.IsInstanceOf<GdUnit3.Executions.ExecutionTimeoutException>().HasMessage("Assertion: Timed out after 700ms."));
        ...
        public async Task RunScene_SimulateKeyPressed()
        {
            ...
            await AssertThrown(spell.AwaitSignal("spell_explode", null).WithTimeout(300))
                .ContinueWith(result => result.Result?.IsInstanceOf<GdUnit3.Executions.ExecutionTimeoutException>().HasMessage("Assertion: Timed out after 300ms."));
