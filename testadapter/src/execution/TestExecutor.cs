...
internal class TestExecutor : BaseTestExecutor, ITestExecutor
{
    ...
    public void Run(IFrameworkHandle frameworkHandle, IRunContext runContext, IEnumerable<TestCase> testCases)
    {
        ...
        //    }) == false)
        //    : testCases;
        string testRunnerScene = Path.Combine(workingDirectory, @$"{temp_test_runner_dir}/TestRunner.tscn");
    ...
    private static void InstallTestRunnerClasses(string destinationFolderPath)
    {
        ...
        string srcTestRunner = """
            ...
            public partial class TestRunner : Api.TestRunner
            {
        ...
        File.WriteAllText(Path.Combine(destinationFolderPath, "TestRunner.cs"), srcTestRunner);

        string srcTestRunnerScene = $"""
            [gd_scene load_steps=2 format=3 uid="uid://5o7l4yufw1rw"]

            [ext_resource type="Script" path="res://{temp_test_runner_dir}/TestRunner.cs" id="1"]
        ...
        """;
        File.WriteAllText(Path.Combine(destinationFolderPath, "TestRunner.tscn"), srcTestRunnerScene);
    }
}
